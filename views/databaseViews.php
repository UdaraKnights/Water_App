...
tl_system_users
WHERE
tl_system_users.approved = '1' AND
tl_system_users.user_name = '{$user}' LIMIT 1";
        ...
        if ($userAvailability > 0) {
			...
			echo json_encode(array(array("udara" => 0, "ss" => "Successfully Logged to the System")));
        }
    } else {
...
if (array_key_exists("userActivation", $_POST)) {
    if ($_POST['userActivation'] == 'active') {
        echo $system->prepareCommandQueryForAlertify("UPDATE `tl_system_users` SET `date`='" . date("Y-m-d") . "', `approved`='1' WHERE (`user_id`='{$_POST['uID']}')", "User Activated for Login", "Sorry..! User Activation failed");
    } 
    else if ($_POST['userActivation'] == 'deactivate') {
        echo $system->prepareCommandQueryForAlertify("UPDATE `tl_system_users` SET `date`='" . date("Y-m-d") . "', `approved`='0' WHERE (`user_id`='{$_POST['uID']}')", "User Deactivated for Login", "Sorry..! User Deactivation failed");
    }
}
...
if (array_key_exists("userLoginProccess", $_POST)) {
    if ($_POST['userLoginProccess'] == 'saveUser') {
        ...
        $modalUserInstitute = $_POST['modelUserIns'];
        $modalUserPassword = $dbClass->PasswordHash($dbClass->filterData($_POST['modalUserPassword']));
        $currentDate = date("Y-m-d");
        echo $system->prepareCommandQueryForAlertify("INSERT INTO `tl_system_users` (`user_name`, `user_level`, `pwd`, `date`, `institute_id`, `approved`) VALUES ('{$modalUserName}', '{$modalUserLevel}', '{$modalUserPassword}', '{$currentDate}','{$modalUserInstitute}', '1')", "Successfully Registered System User", "Sorry..! could not be registered system user");
    } 
    else if ($_POST['userLoginProccess'] == 'deleteUser') {

        $deleteSystemUserID = $_POST['deleteSystemUserID'];
        echo $system->prepareCommandQueryForAlertify("DELETE FROM `tl_system_users` WHERE `user_id` = '{$deleteSystemUserID}'", "Successfully Delete Registered System User", "Sorry..! could not be delete Registered system user");
    } 
        ...
        $modalUserPassword = $dbClass->PasswordHash($dbClass->filterData($_POST['modalUserPassword']));
        $currentDate = date("Y-m-d");
        echo $system->prepareCommandQueryForAlertify("UPDATE `tl_system_users` SET `user_name`='{$modalUserName}', `user_level`='{$modalUserLevel}', `pwd`='{$modalUserPassword}', `date`='{$currentDate}', `institute_id`='{$modalUserInstitute}' WHERE (`user_id`='{$modalUserID}')", "Successfully Update Registered System User", "Sorry..! could not be update registered system user");
    } 
...
tl_system_users
WHERE
tl_system_users.user_id = '{$_POST['userID']}'");
    }
}
