...
if (array_key_exists("logSystem", $_POST)) {
    ...
    if (isset($_POST['userName']) && !empty($_POST['userName']) && isset($_POST['password']) && !empty($_POST['password'])) {
        ...
        $userQuery = "SELECT * from wb_m_users where user_name='$user' Limit 1";
            ...
            foreach ($userDetails as $ud) {
                if ($ud['pwd'] == $dbClass->PasswordHash($pass, substr($ud['pwd'], 0, 9))) {
                    ...
                    if (isset($_POST['remember']) && $_POST['remember'] == 'r') {
                        ...
                        setcookie("HTTP_USER_AGENT", $_SESSION['HTTP_USER_AGENT'], time() + 60 * 60 * 24 * COOKIE_TIME_OUT, "/");
                    }
...
if (array_key_exists("userActivation", $_POST)) {
    if ($_POST['userActivation'] == 'active') {
        echo $system->prepareCommandQueryForAlertify("UPDATE `tl_system_users` SET `date`='" . date("Y-m-d") . "', `approved`='1' WHERE (`id`='{$_POST['uID']}')", "User Was Activated", "Sorry..! Could not be Activated this user");
    } else if ($_POST['userActivation'] == 'deactivate') {
        echo $system->prepareCommandQueryForAlertify("UPDATE `tl_system_users` SET `date`='" . date("Y-m-d") . "', `approved`='0' WHERE (`id`='{$_POST['uID']}')", "User Was Deactivated", "Sorry..! Could not be Deactivated this user");
    }
}
...
if (array_key_exists("userLoginProccess", $_POST)) {
    if ($_POST['userLoginProccess'] == 'saveUser') {
        ...
        echo $system->prepareCommandQueryForAlertify("INSERT INTO `tl_system_users` (`user_name`, `user_level`, `pwd`, `date`, `approved`) VALUES ('{$modalUserName}', '{$modalUserLevel}', '{$modalUserPassword}', '{$currentDate}', '1')", "Successfully Registered System User", "Sorry..! could not be registered system user");
    } else if ($_POST['userLoginProccess'] == 'deleteUser') {

        $deleteSystemUserID = $_POST['deleteSystemUserID'];
        echo $system->prepareCommandQueryForAlertify("DELETE FROM `tl_system_users` WHERE `id` = '{$deleteSystemUserID}'", "Successfully Delete Registered System User", "Sorry..! could not be delete Registered system user");
        ...
        $modalUserPassword = $dbClass->PasswordHash($dbClass->filterData($_POST['modalUserPassword']));
        $currentDate = date("Y-m-d");
        echo $system->prepareCommandQueryForAlertify("UPDATE `tl_system_users` SET `user_name`='{$modalUserName}', `user_level`='{$modalUserLevel}', `pwd`='{$modalUserPassword}', `date`='{$currentDate}' WHERE (`id`='{$modalUserID}')", "Successfully Update Registered System User", "Sorry..! could not be update registered system user");
...
tl_system_users
WHERE
tl_system_users.id = '{$_POST['userID']}'");
    }
}
...
//create the zip archive
    ...
    if ($res === TRUE) {
        ...
        echo '<a class="btn btn-link" href="' . $zipfilename . '">???? ?????? ?????</a>';
    } else {
